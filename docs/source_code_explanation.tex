...
\title{
    \textbf{SSHGuard: Source Code Explanation}\\
...
SSHGuard is a Bash security tool designed to detect repeated failed SSH login attempts.
...
\hline
\texttt{src/sshguard.sh} & Main controller. Reads options, loads configuration, chooses the action, and launches execution modes. \\
\hline
...
\hline
\texttt{scripts/install.sh} & Installs the command as \texttt{sshguard} so it can be executed from anywhere. \\
\hline
\texttt{tests/*.sh} & Light, medium, and heavy test scenarios. \\
\hline
\end{longtable}

\section{Main Program: \texttt{sshguard.sh}}
...
be installed as a symbolic link in \texttt{/usr/local/bin/sshguard}. Even if the user runs
\texttt{sshguard} from another directory, the script can still find its configuration and
library files.
...
The install script prepares the project and creates a command named \texttt{sshguard}.
...
\begin{lstlisting}[style=bashstyle]
LOG_FILE=/tmp/auth-demo.log THRESHOLD=5 sshguard -d -l ~/sshguard-demo-logs
\end{lstlisting}

Run the three execution modes:

\begin{lstlisting}[style=bashstyle]
LOG_FILE=/tmp/auth-demo.log THRESHOLD=5 sshguard -d -s -l ~/sshguard-demo-logs
LOG_FILE=/tmp/auth-demo.log THRESHOLD=5 sshguard -d -t -l ~/sshguard-demo-logs
LOG_FILE=/tmp/auth-demo.log THRESHOLD=5 sshguard -d -f -l ~/sshguard-demo-logs
\end{lstlisting}
...
\hline
Help documentation & Available with \texttt{sshguard -h}. \\
\hline
\end{longtable}

\section{Conclusion}

SSHGuard demonstrates a complete Bash-based security workflow. It combines log parsing,
